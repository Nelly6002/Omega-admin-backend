...
if (!SUPABASE_URL || !SERVICE_KEY) {
  console.error('Please set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in your .env before running this script.');
  process.exit(1);
}
...
async function syncLocalUser(supabaseId) {
  ...
  if (update.rows.length > 0) {
    console.log('Updated existing local user with supabase_id:', update.rows[0]);
...
async function run() {
  try {
    ...
    if (data?.id) {
      ...
      console.log('Admin user is ready. You can now login via the frontend using the admin credentials.');
    } else {
